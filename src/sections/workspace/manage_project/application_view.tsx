...
const ApplicationView = ({ application, setShow }: Props) => {
  ...
  return (
    <div className="sticky max-md:fixed top-[158px] max-md:top-navbar max-md:right-0 w-[55%] max-md:w-full max-h-[70vh] max-md:max-h-screen max-md:h-base max-md:z-50 max-md:backdrop-blur-2xl max-md:backdrop-brightness-90 overflow-y-auto flex flex-col justify-between gap-8 p-8 font-primary text-white border-[1px] max-md:border-0 border-primary_btn rounded-lg max-md:rounded-none max-md:animate-fade_third z-10">
      <div className="w-full flex flex-col gap-10 max-md:gap-8">
            ...
            <div className="w-full flex flex-wrap items-center justify-start gap-2 mt-2">
              {application.user.tags &&
                application.user.tags.map(tag => {
                  return (
                    <div className="flex-center text-xs px-2 py-1 border-[1px] border-primary_btn rounded-md" key={tag}>
                      {tag}
                    </div>
                  );
                })}
        ...
        {application.resume ? (
          <Link
            ...
            target="_blank"
            className="w-64 mx-auto p-2 flex-center bg-primary_comp hover:bg-primary_comp_hover active:bg-primary_comp_active transition-ease-300 cursor-pointer rounded-lg font-medium text-lg"
          >
            View Resume
          </Link>
        ) : (
